...
*   **AI & Personalization (RAG):** Future-proofed with a vector-embedding pipeline (Phase 10.5) that allows the AI to "read" past successful campaigns to suggest better content.
...
*   **A/B Testing Architecture:** Support for multiple variants (Variant A/B) per campaign, each with its own `snapshot_id`, allowing for split-testing subject lines and content.
*   **State Machine:**
...
**How:**
*   **Model Context Protocol (MCP):** A standardized server (`scripts/mcp/`) that allows AI agents to safely "talk" to the database, inspect logs, and audit the schema.
